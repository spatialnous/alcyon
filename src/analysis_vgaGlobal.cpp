...
Rcpp::List vgaAngular(Rcpp::XPtr<PointMap> mapPtr,
                      double radius,
  ...
  return RcppRunner::runAnalysis<PointMap>(
    mapPtr, progress,
    [&nthreads, &radius, &gatesOnly] (
        Communicator *comm, Rcpp::XPtr<PointMap> mapPtr){
      AnalysisResult analysisResult;
      if (nthreads == 1) {
        // original algorithm
        analysisResult = VGAAngular(radius, gatesOnly).run(
          comm, *mapPtr, false);
      } else {
        // openmp algorithm
        analysisResult = VGAAngularOpenMP(
          *mapPtr, radius, gatesOnly,
          nthreads == 0 ?
          std::nullopt :
        std::make_optional(nthreads), true).run(comm);
      }
...
Rcpp::List vgaMetric(Rcpp::XPtr<PointMap> mapPtr,
                     double radius,
                     const Rcpp::Nullable<bool> gatesOnlyNV = R_NilValue,
                     const Rcpp::Nullable<int> nthreadsNV = R_NilValue,
                     const Rcpp::Nullable<bool> copyMapNV = R_NilValue,
  ...
  return RcppRunner::runAnalysis<PointMap>(
    ...
    [&nthreads, &radius, &gatesOnly] (
      ...
      if (nthreads == 1) {
        // original algorithm
        analysisResult = VGAMetric(radius, gatesOnly).run(
          comm, *mapPtr, false);
      } else {
        // openmp algorithm
        analysisResult = VGAMetricOpenMP(*mapPtr, radius, gatesOnly,
                                         nthreads == 0 ?
                                           std::nullopt :
                                           std::make_optional(nthreads), true)
        .run(comm);
      }
      return analysisResult;
    });
...
Rcpp::List vgaVisualGlobal(Rcpp::XPtr<PointMap> mapPtr,
                           int radius,
                           const Rcpp::Nullable<bool> gatesOnlyNV = R_NilValue,
                           const Rcpp::Nullable<int> nthreadsNV = R_NilValue,
                           const Rcpp::Nullable<bool> copyMapNV = R_NilValue,
  ...
  return RcppRunner::runAnalysis<PointMap>(
    ...
    [&nthreads, &radius, &gatesOnly](
      ...
      if (nthreads == 1) {
        // original algorithm
        analysisResult = VGAVisualGlobal(radius, gatesOnly).run(
          comm, *mapPtr, false);
      } else {
        // openmp algorithm
