...
test_that("Load axial data from MetaGraph in C++", {
    ...
    expect_length(metaGraphData$latticeMaps, 0L)
})
...
test_that("Load lattice map data from MetaGraph in C++", {
    fileName <- system.file(
    ...
    expect_length(metaGraphData$latticeMaps, 1L)
})


test_that("Load lattice map data from MetaGraph in R", {
    fileName <- system.file(
        "extdata", "testdata", "simple", "simple_interior.graph",
        package = "alcyon"
    )
