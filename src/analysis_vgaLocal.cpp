...
Rcpp::List vgaVisualLocal(Rcpp::XPtr<PointMap> mapPtr,
  ...
  auto algorithm = NullableValue::castIntGet(algorithmNV,
  ...
  if (algorithm != VGALocalAlgorithm::Standard &&
      algorithm != VGALocalAlgorithm::AdjacencyMatrix)
  ...
  return RcppRunner::runAnalysis<PointMap>(
    ...
    [&nthreads, &algorithm, &gatesOnly](
      ...
      if (algorithm == VGALocalAlgorithm::Standard) {
        if (nthreads == 1) {
          // original algorithm
          analysisResult = VGAVisualLocal(gatesOnly).run(
            comm, *mapPtr, false);
        } else {
          // openmp algorithm
          analysisResult = VGAVisualLocalOpenMP(*mapPtr, nthreads == 0 ?
                                                  std::nullopt :
        ...
        analysisResult = VGAVisualLocalAdjMatrix(*mapPtr, gatesOnly,
                                                 nthreads == 0 ?
                                                   std::nullopt :
                                                   std::make_optional(nthreads),
                                                   true)
