...
Rcpp::List readMetaGraph(
  ...
  for (auto & drawingFile : mgd.drawingFiles) {
    if (verbose) {
      Rcpp::Rcout << " - drawingName: "
                  << drawingFile.first.name << std::endl;
    }
    for (auto it = drawingFile.second.begin();
         it != drawingFile.second.end(); ++it) {
      if (verbose) {
  ...
  for (auto it = mgd.dataMaps.begin();
       it != mgd.dataMaps.end(); ++it) {
    if (verbose) {
  ...
  for (auto it = mgd.shapeGraphs.begin();
       it != mgd.shapeGraphs.end(); ++it) {
    if (verbose) {
      Rcpp::Rcout << " - dataMap name: "
                  << it->getName() << std::endl;
    }
      ...
      Rcpp::List::create(
        ...
        Rcpp::Named("ptr") =
          Rcpp::XPtr<ShapeGraph>(
            new ShapeGraph(std::move(*it)), true)
      )
    );
  }
  for (auto it = mgd.pointMaps.begin();
       it != mgd.pointMaps.end(); ++it) {
    if (verbose) {
      Rcpp::Rcout << " - dataMap name: "
                  << it->getName() << std::endl;
    }
